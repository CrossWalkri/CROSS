...
First, ProPack embeds a five-step MEAL system design process in project design itself, before any project activity begins. MEAL plans are co-developed with implementing partners during project design and finalized before project launch. This is not a post-award reporting requirement; it is a design requirement.

Second, ProPack requires mandatory partner co-design. Implementing partners must actively co-develop MEAL plans, meaning the MEAL system is not designed by CRS and handed down: partners participate in specifying indicators, data collection methods, and accountability mechanisms. This creates a shared ownership structure over the evaluation architecture.

Third, ProPack requires Theory of Change development before indicator selection. Indicators are selected to measure the changes posited by the Theory of Change, not selected first and mapped to a logic model afterward.

Fourth, ProPack requires accountability mechanisms as a formal MEAL system element, including community feedback loops and complaint response mechanisms. These are not separately managed programs; they are part of the MEAL system design.

Fifth, ProPack structures learning into the project cycle through After Action Reviews and real-time learning sessions. Learning is not deferred to final evaluation; it is built into monitoring activities throughout the cycle.

---

## How CROSS Satisfies the ProPack Framework

CROSS addresses the ProPack framework at the level of its four-gate sequence and eleven-field indicator specification.

**Entry specification gate:** ProPack's requirement that the five-step MEAL system be designed before project activities begin is the partner-facing equivalent of CROSS's entry specification gate. CROSS requires that indicators, baseline values, target values, data collection methods, and causality stances be declared before any application opens. A CROSS-conformant program funded through CRS will have its MEAL system fully specified at the entry gate, satisfying ProPack's pre-activity design requirement as a structural consequence of conformance.

**Partner co-design and coherence disclosure:** ProPack's mandatory partner co-design requirement has no direct analogue in CROSS's gate architecture, but it is compatible with and supported by CROSS's coherence disclosure requirement. Coherence disclosure requires programs to document how they relate to other interventions in the same problem space, including those conducted by implementing partners. A CROSS-conformant sub-award program that documents its relationship to the primary CRS program through coherence disclosure is simultaneously satisfying the shared ownership structure that ProPack's co-design requirement is designed to produce.
...
| ProPack MEAL Element | CROSS Provision |
|:--|:--|
| Five-step MEAL system design before project launch | Entry specification gate (full indicator and gate specification before activities begin) |
| Theory of Change before indicator selection | Theory of Change hierarchy (activities, outputs, outcomes, goals in sequence) |
| Partner co-design requirement | Coherence disclosure (relationship to partner programs documented at entry gate) |
| Community feedback and complaint response mechanisms | Coverage gap: not yet named as a required CROSS gate element |
| After Action Reviews and real-time learning | Completion gate evidence review; ongoing monitoring against pre-specified indicators |
| Data quality requirements | Eleven-field indicator specification, Fields 8-9 (data source and collection method) |
| Indicator specification in MEAL plans | Eleven-field indicator specification, Fields 1-5 (name, definition, unit, baseline, target) |
