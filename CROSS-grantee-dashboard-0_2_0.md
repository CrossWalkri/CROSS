...
date: 2026-09-03
license: CC0
status: Working draft. Companion implementation specification synced to CROSS-common-reporting-outcome-standards-schema-0_2_0.md (internal 0.5.8). Supersedes version 0.2.0.
related_documents:
  - standards/CROSS-common-reporting-outcome-standards-schema-0_2_0.md
  - standards/CROSS-grant-configurator-0_2_0.md
changelog:
  - "0.3.2 (2026-09-03): Synced to CROSS 0.5.8. Added the served-population scale trigger (CROSS 0.5.2) to the completion-gate floor: past the Coordination Scaling Standard affected-party reach ceiling, the carried consideration alone does not satisfy the floor and the completion determination records the Structural Power Obligation Standard proxy or guardian channel through which the round handled the population, required before completion above the ceiling. Data model gains above_reach_ceiling and proxy_or_guardian_channel on served_population_floor_considered. The ORE and STRUCK boundary adoptions (CROSS 0.5.4 to 0.5.7) are standard-level Part I declarations, not dashboard fields. Sync stamp advanced 0.5.0 to 0.5.8; the stale 0_5_0 filename references corrected to the 0_2_0 stem in status and related_documents. Frame Language own-voice pass clean; em-dash sweep clean. No existing field or gate behavior changed."
...
Gate_Submission {
  ...
  served_population_floor_considered: {
    ...
    above_reach_ceiling: boolean (true where the served population is past the Coordination Scaling Standard affected-party reach ceiling)
    proxy_or_guardian_channel: text or null (required where above_reach_ceiling is true: the authorized channel through which the round handled the population)
  } or null (null for gate types other than completion_verification)
