#!/usr/bin/env python3
"""Enforce CROSS's structural indicator obligations: the conformant example must validate
against dist/cross.schema.json and every non-conformant fixture must fail. Run against the
GENERATED JSON Schema with a standard 2020-12 validator (not LinkML's own), which is where the
conditional obligations (the Part V baseline, contract-verification, and corroboration rules)
are actually enforced. Exit 0 if all pass, 1 otherwise."""
import json, sys
from pathlib import Path
import yaml, jsonschema

HERE = Path(__file__).resolve().parent.parent
schema = json.loads((HERE / "dist" / "cross.schema.json").read_text())
ok = True
def must_pass(p):
    global ok
