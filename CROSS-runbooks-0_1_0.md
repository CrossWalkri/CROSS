...
**Sufficiency architecture declaration.** Every entry gate submission must include the sufficiency architecture declaration alongside the organizational identity declaration: the scope declaration, the sufficiency position (critical gap, partial, approaching, or surplus at the declared scope), the portfolio context statement covering how other funding addresses other aspects of the entity's work, and the grant contribution statement specifying what gap this grant closes.

**Unintended outcomes disclosure.** Every active completion verification gate must collect an unintended outcomes disclosure alongside primary outcome evidence. Grantees must identify any effects, positive or negative, occurring during the grant period that are plausibly connected to the funded work and were not specified at entry. Silence does not satisfy the requirement; the grantee must explicitly state whether unintended outcomes occurred. Positive unintended outcomes are recorded as discoveries; negative ones trigger the Adverse-Signal Engagement Principle at the continuation gate.

**Structured dataset publication.** At the close of each round, the program must publish a machine-readable structured dataset containing the indicator specifications, gate configurations, evidence scope and strength at each gate, obligation modes, sufficiency positions, causality stances, sustainability stances at continuation, and unintended outcomes disclosures for all funded grants. Where WALKRI has been adopted, this is automatically satisfied through the field-level metadata WALKRI-conformant fields carry.

**Round specification pre-commitment.** Programs whose gate architecture includes summative completion gates with declared redress rights are strongly advised to publish a verifiable commitment to the round specification before the application window opens. The pre-commitment record is the primary evidentiary basis for applicant redress rights in procedural appeals.

---

## Configuration-Activated Requirements

The following requirements are activated by specific gate configuration choices rather than universally. Each runbook's configuration interaction notes identify which apply.

**Prior Work Attribution Statement.** Where an applicant cites prior work as evidence of capability, track record, or past contribution in any gate submission, a prior work attribution statement is required naming: the entity under which the work was performed, the applicant's relationship to that entity at the time, and the current ownership and access status of the IP, codebase, or user relationship cited.

**Reviewer calibration.** Any gate configured at independent review level or above requires a reviewer calibration exercise before the review window opens. The exercise must present reviewers with worked examples (one clear pass, one clear fail) and include an inter-rater consistency check where multiple reviewers assess the same applications. The calibration record must be retained as part of the gate evidence record. This requirement applies to all runbooks that configure independent review at any gate.

**Causality stance declaration.** Gates submitting outcome evidence must declare whether the gate is configured at attribution stance (requiring counterfactual evidence including a counterfactual baseline, attribution argument, comparison period or group, and independent attestation) or contribution stance (requiring a named causal pathway, co-factors, and a materiality argument). The stance is declared in the gate configuration and published before the round opens.

**Sustainability stance at continuation.** Where a continuation gate is configured for a change-obligation program, the continuation submission must include a sustainability stance on outcomes produced in prior rounds: sustained (outcomes continue under their own momentum), conditional (outcomes persist under specified conditions independent of this program), or dependent (outcomes require continued intervention to persist). A dependent stance is a disclosure, not a disqualifier.

**Pre-Award Indicator Confirmation Window.** An optional bounded window after entry gate passage and before first disbursement during which the funder reviews and confirms or requests revision of the indicator specification submitted at entry. Silence at the window's close constitutes confirmation. Strongly recommended for Runbooks 5 and 6.

**Gate Record Publication.** Where gate determinations are expressed as publicly verifiable signed records, subsequent programs may query those records as corroboration for the organizational identity declaration's prior round history field. Programs are encouraged to express gate determinations in a format queryable without grantee cooperation.

---

## Runbook Configuration Interaction Notes

The following notes identify, for each runbook, which configuration-activated requirements apply and any runbook-specific interaction between the universal requirements and the runbook's gate configuration.

**Runbook 1 (Discovery Sprint).** Prior work attribution applies where the applicant cites prior research, prior deliverables, or prior community engagement as evidence of capacity. Where the deliverable is a research artifact, novel algorithm, dataset, or other output with IP implications, the IP ownership declaration from Part II applies. The pre-commitment is recommended given the completion gate is summative and final payment depends on the completion determination.

**Runbook 2 (Staged Development).** Prior work attribution applies at Stage 1 entry where the team cites prior work to establish capacity. Causality stance must be declared if change obligation is activated at Stage 2. Sustainability stance is required at the Stage 1 to Stage 2 continuation gate if change obligation is active at Stage 2. Reviewer calibration is required at Stage 2 if the completion verification gate is configured at independent review level. The pre-commitment is recommended at Stage 2 given the summative character of the Stage 2 completion gate.

**Runbook 3 (Community Allocation with Build Obligation).** The sufficiency architecture declaration and organizational identity declaration apply to all applicants regardless of grant size or allocation mechanism. For programs using sybil resistance mechanisms (Gitcoin Passport score threshold or comparable systems), the score threshold is a conformant entry gate eligibility criterion and must be declared in the round configuration with the scoring model version, threshold value, and measurement date. The community allocation mechanism governs how funds are allocated among eligible applicants; it does not replace the gate architecture governing what applicants are obligated to demonstrate.

**Runbook 4 (Retroactive Impact).** Prior work attribution applies to all applicants since the entire entry submission consists of prior work citations. The reviewer panel constitutes the independent review mechanism at the entry specification gate; reviewer calibration is therefore required before any panel review begins. Causality stance should be declared for the evidence at the entry gate: most retroactive programs operate at contribution stance, but the stance must be explicit in the rubric rather than implied. The pre-commitment is strongly advised since retroactive rounds frequently face disputes about whether criteria changed after submission.

**Runbook 5 (Graduated Evidence).** Causality stance must be declared at Stage 2 completion if change obligation is active, and is required at the Stage 2 to Stage 3 continuation gate and the Stage 3 completion gate. Sustainability stance is required at the Stage 2 to Stage 3 continuation gate. Reviewer calibration is required at Stage 2 independent review and at Stage 3 independent evaluation. The Pre-Award Indicator Confirmation Window is strongly recommended at Stage 2 entry, where indicators specified in Stage 1 typically require refinement before Stage 2 work begins. The pre-commitment is recommended for Stage 2 and Stage 3 given their summative completion characters and the scale of the funding at stake.

**Runbook 6 (Institutional Conformance).** Causality stance is required at the completion verification gate and at the continuation specification gate where configured; institutional programs typically operate at attribution stance, requiring counterfactual evidence. Sustainability stance is required at every continuation gate. Reviewer calibration is required at the environmental and social specialist review, at completion verification independent review, and at any continuation specification independent evaluation. The Pre-Award Indicator Confirmation Window is strongly recommended given the complexity of institutional indicator specifications and the cost of disputes at the completion gate. The pre-commitment is strongly advised; institutional redress mechanisms depend on the ability to demonstrate that criteria were published before applications were received.
...
|---|---|---|
| 0.2.0 | 2026-05-16 | Major update to align with CROSS v0.3.2. Added Universal Gate Requirements section covering seven requirements that apply to all runbooks: gate criterion specification (WALKRI), organizational identity declaration, sufficiency architecture declaration, unintended outcomes disclosure, structured dataset publication, round specification pre-commitment, and gate record publication. Added Configuration-Activated Requirements section covering six requirements that activate by gate configuration: prior work attribution, reviewer calibration, causality stance declaration, sustainability stance at continuation, pre-award indicator confirmation window, and gate record publication guidance. Added Runbook Configuration Interaction Notes section with targeted notes for each of the six runbooks identifying which configuration-activated requirements apply and any runbook-specific interactions. Version references updated from CROSS v0.2.0 to CROSS v0.3.2. |
