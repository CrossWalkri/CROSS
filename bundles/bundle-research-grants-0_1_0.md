...
SBIR/STTR and ARPA programs use portfolio-level benchmarks that aggregate across individual grants, creating a distinct obligation layer above the grant level:
...
|---|---|---|
| 0.1.1 | 2026-06-12 | Frame Language own-voice pass applied as part of the bundle long-tail sweep. Three own-voice recasts: "the pre-commitment to a transparent process" to "a legible process"; "a distinct accountability layer above the grant level" to "a distinct obligation layer"; "Programs building research accountability infrastructure" to "research obligation infrastructure." Citation use preserved: Arnold Ventures Open Science Guidelines (protocol transparency as its funder condition), PCORI's mandatory patient and stakeholder engagement. No field, conformance threshold, or compatibility mapping changed; vocabulary only. Em-dash- and watchlist-clean re-verified. |
